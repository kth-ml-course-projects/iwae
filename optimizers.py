...
class Adam:
    ...
    def updatesIgrad_model(self, grad, model):
        ...
        first_moments_updates = collections.OrderedDict([
            (self.first_moments[param],
             self.beta_1 * self.first_moments[param] + (1 - self.beta_1) * grad[param])
            for param in model.params])

        second_moments_updates = collections.OrderedDict([
            (self.second_moments[param],
        ...
        sgd_updates = collections.OrderedDict([(param,
                                                param + alpha * self.first_moments[param] / (
                                                        T.sqrt(self.second_moments[param]) + 1e-4))
                                               for param in model.params])
